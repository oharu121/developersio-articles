...
記事を書き始める前に、プロジェクトのルートディレクトリにある既存のフォルダ一覧を確認し、番号付きメニューを**1つだけ**表示してユーザーに選んでもらう。

1. プロジェクトルートのサブディレクトリを一覧表示する
2. 以下の候補をまとめた**単一の番号付きメニュー**を表示する:
   - トピックに関連する既存フォルダ（`(既存)` ラベル付き）
   - トピックに基づいた新規フォルダ名の提案（1-2個、`[新規作成]` ラベル付き、kebab-case で簡潔に）
   - 最後に「その他（フォルダ名を自由入力）」の選択肢
   - 既存フォルダに関連するものがない場合は、新規作成の候補と自由入力のみ表示する
3. ユーザーが番号またはフォルダ名で回答したら、選択されたフォルダが存在しない場合は作成する
4. トピックに関連するタグの候補も合わせて提案する（ユーザーは記事作成後に変更可能）
5. そのフォルダ内に記事のMarkdownファイルを作成する（ファイル名は `{slug}.md`）
